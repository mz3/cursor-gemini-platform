...
export const seedDatabase = async (): Promise<void> => {
  try {
    ...
    if (fixtures.secrets) {
      for (const secretData of fixtures.secrets) {
        try {
          ...
          // Create secret with encrypted value from fixture
          ...
          // Set the encrypted value from the fixture
          if (secretData.encryptedValue) {
            secret.encryptedValue = secretData.encryptedValue;
          } else {
            // Fallback to dummy value if no encrypted value provided
    ...
    // Create AI models from fixtures
    if (fixtures.aiModels) {
      for (const aiModelData of fixtures.aiModels) {
        try {
          // Use email reference to get actual user ID from mapping
          const userId = idMapping.users[aiModelData.userId];
          if (!userId) {
            console.warn(`User not found for AI model ${aiModelData.name}: ${aiModelData.userId}`);
            continue;
          }

          const aiModel = aiModelRepository.create({
            ...aiModelData,
            userId: userId // Use mapped user ID
          }) as any;
